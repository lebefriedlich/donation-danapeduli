...
import type { Campaign, CampaignFilterType, CampaignType } from '@/types';
...
function Badge({ type }: { type: CampaignType }) {
    ...
    const cls = isCrowd ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : 'bg-sky-100 text-sky-700 border-sky-200';

    return (
        <span className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-xs font-semibold ${cls}`}>
            <span className={`h-1.5 w-1.5 rounded-full ${isCrowd ? 'bg-emerald-600' : 'bg-sky-600'}`} />
    ...
    return (
        ...
        >
            <div className="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
                ...
                <div className="p-5">
                    <h3 className="line-clamp-2 text-base font-bold text-slate-900">{c.title}</h3>
                    ...
                    {hasTarget ? (
                        ...
                        <div className="mt-3 text-xs text-slate-500">
                            Terkumpul: <span className="font-semibold text-slate-700">Rp {formatIDR(c.total_paid)}</span>
                        </div>
                    )}
                    ...
                    <div className="mt-4 flex items-center justify-between">
                        ...
                        <span className="text-xs text-slate-500">
                            Status: <span className="font-semibold text-slate-700">{c.status === 'ACTIVE' ? 'Aktif' : c.status}</span>
                        </span>
                    </div>
                </div>
            </div>
...
export default function Index({ type, campaigns }: Props) {
    const activeType: CampaignFilterType = type;

    const switchType = (nextType: CampaignFilterType) => {
        router.get('/', { type: nextType }, { preserveScroll: true, preserveState: true });
    };
    ...
    return (
        <PublicLayout>
            <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <h1 className="text-2xl font-black tracking-tight text-slate-900">Program Donasi</h1>
                    <p className="mt-1 text-sm text-slate-600">Pilih program yang ingin kamu dukung.</p>
                </div>
                ...
                <div className="flex items-center gap-2">
                    <button
                        ...
                        className={`rounded-full px-4 py-2 text-sm font-semibold transition ${
                            activeType === 'ALL' ? 'bg-slate-900 text-white' : 'border border-slate-200 bg-white text-slate-700 hover:bg-slate-100'
                        }`}
                    >
                        Semua
                    </button>
                    ...
                    <button
                        ...
                        className={`rounded-full px-4 py-2 text-sm font-semibold transition ${
                            activeType === 'DONATION' ? 'bg-sky-600 text-white' : 'border border-slate-200 bg-white text-slate-700 hover:bg-slate-100'
                        }`}
                    >
                        Donasi
                    </button>
            ...
            <section className="mt-8">
                {campaigns.length === 0 ? (
                    <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center">
                        <p className="text-sm text-slate-600">Belum ada campaign yang tersedia.</p>
                    </div>
                ) : (
